"""
NASA Turbofan-inspired sensor data generator for the Predictive Maintenance System.
Simulates CMAPSS-style data with temperature, vibration, pressure, and RPM sensors.
"""
...
def compute_health_score(cycle: int, max_cycle: int, degradation_rate: float) -> float:
    """Nonlinear health degradation inspired by NASA CMAPSS."""
...
def simulate_sensor_reading(sensor: str, health: float) -> float:
    """Generate sensor reading with degradation signal."""
...
def generate_dataset(readings_per_machine: int = 50) -> pd.DataFrame:
    """Generate full dataset simulating N machines over time."""
    records = []
...
def get_latest_snapshot() -> pd.DataFrame:
    """Return the most recent reading per machine."""
