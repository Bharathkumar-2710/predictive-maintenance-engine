"""
OOP class hierarchy for Sensor types and Alert levels.
Models real industrial sensor taxonomy with calibration, validation, and thresholds.
"""
...
class AlertSeverity(Enum):
    ...
    def from_health(cls, health_score: float) -> "AlertSeverity":
        if health_score >= 0.75:  return cls.NORMAL
        if health_score >= 0.50:  return cls.WATCH
        if health_score >= 0.25:  return cls.WARNING
        return cls.CRITICAL
...
@dataclass
class Alert:
    """Represents a triggered alert for a machine at a point in time."""
    machine_id:   str
...
class Sensor(ABC):
    ...
    def validate(self, value: float) -> bool:
        """Return True if the reading is physically plausible."""
        ...
...
class TemperatureSensor(Sensor):
    """Thermocouple-style temperature sensor (°F, turbine/bearing monitoring)."""
    ...
    @property
    def warning_threshold(self) -> float: return 490.0

    @property
    def critical_threshold(self) -> float: return 530.0
...
class VibrationSensor(Sensor):
    """Accelerometer-based vibration sensor (g-force, imbalance/bearing wear detection)."""

    def __init__(self, sensor_id: str, machine_id: str):
        super().__init__(sensor_id, machine_id, unit="g")

    @property
    def name(self) -> str: return "vibration"

    @property
    def warning_threshold(self) -> float: return 0.45

    @property
    def critical_threshold(self) -> float: return 0.60
...
class PressureSensor(Sensor):
    """Piezoelectric pressure transducer (PSI, oil/hydraulic systems)."""

    def __init__(self, sensor_id: str, machine_id: str):
        super().__init__(sensor_id, machine_id, unit="PSI")

    @property
    def name(self) -> str: return "pressure"

    @property
    def warning_threshold(self) -> float: return 10.0   # too low = leak/wear

    @property
    def critical_threshold(self) -> float: return 7.5

    def validate(self, value: float) -> bool:
        return 0.0 <= value <= 200.0

    def check_alert(self, value: float) -> Optional[Alert]:
        """Pressure alerts fire when BELOW thresholds (inverted logic)."""
        if not self.validate(value):
...
class RPMSensor(Sensor):
    """Hall-effect tachometer (RPM, rotational speed monitoring)."""

    def __init__(self, sensor_id: str, machine_id: str):
        super().__init__(sensor_id, machine_id, unit="RPM")

    @property
    def name(self) -> str: return "rpm"

    @property
    def warning_threshold(self) -> float: return 1650.0   # too low = degraded

    @property
    def critical_threshold(self) -> float: return 1500.0
...
class OilViscositySensor(Sensor):
    """In-line viscometer (cSt, lubrication quality monitoring)."""

    def __init__(self, sensor_id: str, machine_id: str):
        super().__init__(sensor_id, machine_id, unit="cSt")

    @property
    def name(self) -> str: return "oil_viscosity"

    @property
    def warning_threshold(self) -> float: return 36.0

    @property
    def critical_threshold(self) -> float: return 30.0
...
class CurrentDrawSensor(Sensor):
    """Clamp ammeter for motor current draw (Amps, electrical health monitoring)."""

    def __init__(self, sensor_id: str, machine_id: str):
        super().__init__(sensor_id, machine_id, unit="A")

    @property
    def name(self) -> str: return "current_draw"

    @property
    def warning_threshold(self) -> float: return 15.5

    @property
    def critical_threshold(self) -> float: return 17.0
...
def create_sensor(sensor_type: str, machine_id: str) -> Sensor:
    """Factory function — instantiates the correct Sensor subclass."""
